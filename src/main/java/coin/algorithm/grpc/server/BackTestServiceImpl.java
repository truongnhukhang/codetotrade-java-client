...
public class BackTestServiceImpl extends CoinAlgorithmServiceGrpc.CoinAlgorithmServiceImplBase {
  ...
  public void createBackTest(
      CreateBackTestRequest request, StreamObserver<CreateBackTestResponse> responseObserver) {
    ...
    backTestData.setOtherBarSeries(
        request.getCandleMapMap().entrySet().stream()
            .collect(
                Collectors.toMap(
                    e -> Duration.ofMinutes(e.getKey()),
                    e -> BackTestData.fromCandles(e.getValue().getCandlesList()))));
    backTestData.setBtcDomBarSeries(
        request.getBtcDomCandleMapMap().entrySet().stream()
            .collect(
                Collectors.toMap(
                    e -> Duration.ofMinutes(e.getKey()),
                    e -> BackTestData.fromCandles(e.getValue().getCandlesList()))));
    ...
    CreateBackTestResponse response =
        CreateBackTestResponse.newBuilder().setBackTestId(uuid.toString()).build();
    responseObserver.onNext(response);
    responseObserver.onCompleted();
  }

  @Override
  public void getTimeTravel(
      GetTimeTravelRequest request, StreamObserver<GetTimeTravelResponse> responseObserver) {
    ...
    try {
      ...
      GetTimeTravelResponse response =
          GetTimeTravelResponse.newBuilder()
              .addAllSignal(
                  rs.getSignals().stream()
                      .map(ProtoConverter::toProtoSignal)
                      .collect(Collectors.toList()))
              .addAllTradeMetadata(
                  rs.getMetadata().stream()
                      .map(ProtoConverter::toProtoTradeMetadata)
                      .collect(Collectors.toList()))
              .setChartList(
                  ChartList.newBuilder()
                      .addAllCharts(
                          charts.stream()
                              .map(c -> ProtoConverter.toProtoChart(c, backTestData))
                              .collect(Collectors.toList())))
              .build();
      responseObserver.onNext(response);
      responseObserver.onCompleted();
